...
direct an animated film end to end — source the world, stage it, animate
...
1. **Source real assets before geometry** fetch 3D models, PBR textures, and
   HDRI environments from open libraries and local assets, then read the generated 
   preview of every mesh before placing it.
2. **Build procedural geometry** build geometry out of primatives or loft geometry
   for use in the scene, texture said geometry procedurally or with pre-existing textures
3. **Stage with intent** placement helpers (`placeOn`, `placeAgainst`,
   `snapToGround`, `scatterOn`) seat props on surfaces and against walls
   instead of hand-guessed coordinates.
4. **Animate** VRM characters with a locomotion controller, foot IK, default
   animation clips, facial expressions, and viseme-based lip sync.
5. **Score it** generative music and narration tracks, mixed under the runtime.
6. **Render** Deno + WebGPU, real-time-ish on a modern GPU.
7. **Verify** the pipeline reads its own output back and audits for the
